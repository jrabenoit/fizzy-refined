...
def Bestimator():
    ...
    for j,k in zip(estimators.keys(), estimators.values()):           
        ...
        hresults['hestimator'].extend([j]*len(hX))
        hresults['hsubjects'].extend(hlabels.index)
        hresults['hlabels'].extend(hy)
        hresults['hpredictions'].extend(hpredict)
        hresults['hscores'].extend(hscores)
        hresults['hattempts'].extend([1]*len(hX))

    df=pd.DataFrame.from_dict(results).set_index('subjects')
    hdf=pd.DataFrame.from_dict(hresults).set_index('hsubjects')
    ...
    hestimator= hdf.groupby('hestimator').sum()
    haccuracy= (hestimator['hscores']/hestimator['hattempts'])*100
    print(haccuracy)
    pmax= haccuracy.idxmax(axis=1)
    print('\nHoldout set haccuracy: {}\n'.format(pmax))
